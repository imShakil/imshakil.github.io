  ...
  useEffect(() => {
    const fetchActivities = async () => {
      try {
        ...
        // Fetch Medium posts
        try {
          const mediumResponse = await fetch('/api/activities/medium');
        ...
        try {
          const youtubeResponse = await fetch('/api/activities/youtube');
    ...
    <main className="min-h-screen bg-white dark:bg-slate-950 flex flex-col">
      ...
      <section className="flex-1 py-20 px-6 md:px-20">
        <div className="max-w-6xl mx-auto">
          {error && (
            <div className="bg-red-100 dark:bg-red-500/10 border border-red-200 dark:border-red-500/30 text-red-700 dark:text-red-400 p-4 rounded-lg mb-8">
              {error}
            </div>
          )}
